...
export const AdminSettings: React.FC = () => {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setSaving(false);
      addToast('Administrative platform configurations successfully active.', 'success');
    }, 600);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col gap-8 text-left animate-fade-in">
      <div>
        <h2 className="text-2xl font-black text-on-surface font-display">System Platform Settings</h2>
        <p className="text-sm text-on-surface-variant mt-1">Configure global server nodes, backups rates, and client theme overrides.</p>
      </div>
      ...
      <div className="glass-card p-6 md:p-8 rounded-2xl border-outline-variant/30 flex flex-col gap-6 bg-white dark:bg-surface-container">
        <form onSubmit={handleSave} className="flex flex-col gap-6">
          ...
          <Input 
            label="Core System Node URL" 
            value="https://api.gradeai.university.edu" 
            disabled
          />
